...
PaykounContext.prototype.setType = function(type) {
  assert.ok(!this.isRunning, "Can't change PaykounContext type after running it.");

  this.type = type;
};

PaykounContext.prototype.createRunnerFactory = function(type) {
  var opts = {type: type};

  return function(runOpts, done){
    var clone = _.clone(runOpts);

    return JobRunner.create(_.defaults(clone, opts), done);
  };
};
...
PaykounContext.prototype.createJobRunners = function(params, callback) {
  ...
  var runnerFactory = this.createRunnerFactory(this.type);

  runnerFactory = _.wrap(runnerFactory, function(wrapped, opts, done){
    wrapped(opts, function(err, jobRunner){
