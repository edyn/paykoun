...
var poolSize = process.env.DEFAULT_POOL_SIZE || 10;
  ...
  this._workQueueMgr.on('ready', function(err){
    ...
    _.each(self._allWorkers, function(worker, key){
      ...
      vasync.waterfall([
        ...
        self.createThreadPools.bind(self)
      ], function(err, res){
...
PaykounContext.prototype.createThreadPools = function(arg, callback) {
  this.threadPool= Threads.createPool(poolSize);

  var workFunctions = {}
  this.threadPool.load(__dirname + '/pool_thread.js');
  var toEval = createBootstapFunc(this.workersByName);

  thread.all.eval(toEval);
};
